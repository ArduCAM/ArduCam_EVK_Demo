...
def main(config_path: str, device_index: int):
    camera = Camera()
    param = Param()
    devs = list_devices()  # get all devices
    device_index -= 1  # make the index start from 0
    if device_index < 0:
        raise Exception(f"device index must be greater than 1.")
    if device_index >= len(devs):
        raise Exception(f"device index out of range, total {len(devs)} devices.")
